...
export function mockEventExpand(rawArgs: unknown, rng: Rng): ExpandOut {
  ...
  const p3 =
    effectSummaries.length > 0
      ? `What followed can be traced forward through the ledger: ${effectSummaries[0]?.toLowerCase().replace(/\.$/, '')}. At the time, though, the persistent fact was simpler — ${stateFact.toLowerCase()} — and everything else was adjustment to it.`
...
export function mockEraDeepDive(rawArgs: unknown, rng: Rng): ExpandOut {
  ...
  const middle = `The record answers through its events. ${eventLines
    ...
    .join(
      '; ',
    )} — each a partial discharge of the same accumulated pressure, none a settlement. ${texture}`
