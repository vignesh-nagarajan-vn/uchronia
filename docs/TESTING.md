...
```sh
pnpm test          # vitest across all packages (~145 tests)
...
- **core unit** (85): seeded rng · dial mapping (bands, budgets, floors) · store — change-log replay, multi-level fork resolution, mutation guards incl. pre-fork immutability · every validator rule individually · structured repair loop (success/repair/fail-loud/non-JSON) · mock pod intake determinism · era planning.
- **core integration**: full pipeline in mock (seed + era loop to the horizon, determinism, resume-after-interrupt), dual-review paths (regenerate-and-fix, dispute-kept, machine-drop, wildcard floor), convergence against the real baseline, lazy expansion fill-once semantics, multi-level fork generation with sub-PODs.
- **server** (30): routes with the injected MockProvider — CRUD, import/export round-trip, branch views through forks, SSE lifecycle **including client abort mid-stream**, expansion persistence, fork/compare, artifacts, md/html exporters.
...
dates-monotonic · event-within-era · edge-endpoints-exist · entities-exist · deltas-apply · plausibility-range · era-overlap · fork-normalized (+ store-level pre-fork immutability guards).
