...
2. **Playable demo**: two shapes, both defaulting to mock mode. The Dockerfile (one container serves web and API via `UCHRONIA_STATIC_DIR` + SPA fallback; history persists in a `/data` volume), and a zero-config Vercel setup (`vercel.json` running the whole Hono app as one function — `api/index.ts` when this was recorded, since prebundled behind `api/index.mjs`; SQLite in `/tmp` per instance, showcase chronicle seeded on cold start). Serverless state is deliberately ephemeral: a playground, not an archive. Realized at <https://uchronia-server.vercel.app/> (2026-07-26).
