...
- Live-mode generation is wired, provider-unit-tested, and cost-capped, and has still never been exercised from this machine (no key present); mock parity remains the tested path here. **The deployment is now where it gets exercised, and the intake half passes there** (2026-08-01): POD interpretation returns real readings on the right year and region, `zodOutputFormat` accepts all 29 prompt schemas, and the metering added in M26 charges both ledgers correctly against real usage. Still unexercised live: the era loop, the symposium, the court, the expanders, and the archivist. The Engine Room and the cost meter are the instruments for reading them.
- **One unexplained 500 on the deployment's first live interpretation (2026-08-01), not reproducible since.** It failed past the gate and past the key (the live check passed throughout), fell through every mapped error to the catch-all, and then stopped happening. The commit that made unmapped provider failures legible (`ProviderUnknownError`) only changed reporting, so it cannot have been the cure: the cause is genuinely unknown, and the leading guess is first-call structured-output schema compilation colliding with a cold start. Recorded rather than closed. If it recurs it now surfaces as a 502 naming the throw instead of an anonymous 500.
