...
**The WW2 gate, demo half: passes** (card offers the real mechanisms, POD lands 1939-1945, `demo/the-allies-lose.uchronia.json` runs 1940-2024 on subject). **Live half: the intake now passes on the deployment** (2026-08-01, per ADR-0004's plan to verify there rather than here): "What if the allies lost WW2?" interprets live to 1940 in Europe, and "What if Constantinople held in 1453?" to 1453 in the Mediterranean. Metering was confirmed at the same time, charging the day's ledger and the visitor's share identically. No key has ever been in this tree; the live eval lane is still one command from running on a machine that has one. **Build-time API spend for the whole v2 program: zero.**
